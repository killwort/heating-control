...
enum class OpenThermResponseStatus : unsigned char
{
    NONE,
    SUCCESS,
    INVALID,
...
enum class OpenThermMessageType : unsigned char
{
    /*  Master to Slave */
    READ_DATA = 0b000,
...
enum class OpenThermMessageID : unsigned char
{
...
enum class OpenThermStatus : unsigned char
{
    NOT_INITIALIZED,
    READY,
    DELAY,
...
{
public:
    OpenTherm(const char* chipDevPath, int inPin = 18, int outPin = 200, bool isSlave = false);
    ~OpenTherm();
    ...
    void handleInterrupt(::gpiod::line_event& ev);
    void process();
    void end();
...
private:
    ...
    volatile unsigned char responseBitIndex;
